...
**Date:** 2026-06-23
**Repo (briefs live here):** `sublimecoding-marketing`
...
| # | Working title | Primary query | Funnel target |
|---|---|---|---|
| 1 | Prompt injection risks in Claude Code CI/CD | `prompt injection claude code ci/cd` | Consulting (security) |
| 2 | CLAUDE.md vs AGENTS.md: what a staff engineer actually uses | `claude.md vs agents.md` | Vault (CLAUDE.md starters) |
| 3 | The AI coding workflow of a staff engineer | `ai coding workflow staff engineer` | Vault (all 15 prompts) |
| 4 | Cursor vs Claude Code for backend engineers | `cursor vs claude code backend` | Vault + Consulting |
| 5 | .cursorrules vs CLAUDE.md: which to check into your repo | `.cursorrules vs claude.md` | Vault (.cursorrules) |
| 6 | How to write a CLAUDE.md that works in production | `how to write claude.md` | Vault (CLAUDE.md starters) |
...
- **#6** overlaps `claude-md-after-50-commits` (personal/mistake-driven). Recon decides: reframe #6 as a distinct how-to/guide targeting `how to write claude.md` with reciprocal cross-link, OR fold into the existing post. Default: reframe + cross-link; the brief states the chosen wedge.
...
- Briefs: `content-briefs/<slug>.md` in **this** marketing repo (SEO program self-contained). One brief = one file = one writing unit, independently understandable.
- Published posts (later, out of scope): `../sublimecoding-site/content/posts/<slug>.md`.
- A short `content-briefs/README.md` indexes the 6 briefs, their target queries, and status.
...
## Success criteria
- 6 brief files in `content-briefs/`, each containing all 9 structure elements with **real fresh-SERP data** (not generic guesses).
- Each brief's frontmatter uses the existing tag taxonomy and a valid canonical URL pattern.
- Each brief resolves its cannibalization decision explicitly and lists reciprocal interlinks.
- `content-briefs/README.md` index present.
...
- Any change to `../sublimecoding-site` (briefs only reference it).
- The Go/Elixir/security `.cursorrules` spokes (separate follow-up from Phase 1).
- OG images for posts.

## Open items to confirm at review
1. Brief location: `content-briefs/` in this repo (vs. site repo `content/briefs/`).
2. The #6 cannibalization default (reframe + cross-link vs fold-in) — recon recommends, but confirm the default stance.
3. Whether to set tentative `publishDate`s (a cadence) or leave as placeholders.
